...
is similar to LOD (Level of Detail), but is used for textures. It reduces the textures resolution when the texture
...
surface class that returns the extent. We will also add a conversion operator to the device class that
returns the device.
...
MSAA (Multisample Anti-Aliasing) is handled. Let's rewind shortly how MSAA works.
...
But this returns a jagged edge. To fix this we can use MSAA. MSAA works by sampling multiple points in the pixel
...
GPU access. We will use the optimal tiling mode for the depth format. We will check the following formats:
...
depth image and one depth image view for each swapchain image. The problem is that when we create the depth image on the CPU we
need to allocate memory on the GPU and then bind the image to that memory, because we need the data there.
...
GLSL language for the shaders. Let's create a new folder in our src folder called shaders. In this folder we will create
...
GLSL looks very similar to C++ but has some differences. For example we have to define the version of the GLSL language
...
use the \textit{glslc} compiler that comes with the Vulkan SDK. The \textit{glslc} compiler takes the shader file and then compiles
...
between a shader module and a shader stage is, the shader module is the binary data that is loaded into the GPU and the
...
created with any strip topology. We don't need this for now, so we will set it to VK\textunderscore FALSE.
...
polygons. Any mode other than the fill mode will need you to activate a GPU feature like the
\textit{VK\textunderscore EXT\textunderscore line\textunderscore rasterization} feature.
...
need this so we can set it to VK\textunderscore FALSE and ignore the depth bound members. Finally we can also enable the stencil
...
a descriptor set layout and create the pipeline layout. The \textit{createPipeline} function will take the render pass and
create the graphics pipeline. We will also need a variable that stores the pipeline
layout, the pipeline and the device. The render system's constructor takes the device reference, render pass and
the descriptor set layout and first creates the pipeline layout and then the graphics pipeline.
